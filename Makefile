...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "fletchR", "tests")); system.time(test_check("fletchR", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran fletchR_$(VERSION).tar.gz
	rm -rf fletchR.Rcheck/
